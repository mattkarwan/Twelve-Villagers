...
let thirdHistory = document.getElementById("thirdHistory");
...
let TH_template = document.getElementById("TH_template");
...
let TH_leftMeeples = document.querySelectorAll(".TH_left-meeple");
let TH_rightMeeples = document.querySelectorAll(".TH_right-meeple");
...
function clearHistory() {
  ...
  thirdHistory.classList.add("invisible");

  setTimeout(() => {
    clearMeeples(FH_leftMeeples, FH_rightMeeples, SH_leftMeeples, SH_rightMeeples, TH_leftMeeples, TH_rightMeeples);
    FH_template.style.transform = "rotate(0deg)";
    SH_template.style.transform = "rotate(0deg)";
    TH_template.style.transform = "rotate(0deg)";
  }, 250);
}
...
function useSeesaw(leftWeight, rightWeight) {
  ...
  // Make meeples not on the seesaw invisible
  ...
  setTimeout(() => {
    ...
    // Apply seesaw animation depending on heavier side

    if (leftWeight === 1 || rightWeight === 2) {
      seesaw.classList.add("lighter");
      outcome = "lighter";
    }
    
    else if (leftWeight === 2 || rightWeight === 1) {
...
function generateHistory (historyNo, outcome) {
  ...
  if (historyNo === firstHistory) {
  ...
  else if (historyNo === secondHistory) {
    ...
    generateHistoryMeeples(leftDropped, TH_leftMeeples, "TH_text");
    generateHistoryMeeples(rightDropped, TH_rightMeeples, "TH_text");
    rotateHistory(outcome, TH_template);
  }
};
...
function saveHistory(outcome) {
  ...
  if (remaining === 3) {
    ...
    setTimeout(() => {
      makeVisible(thirdHistory, weighBtnHolder, weighBtnHolder, meepleHolder, openGuessBtn, nav);
    }, 3000);
    generateHistory(thirdHistory, outcome);
  }
